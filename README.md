...
- **Backend API** на FastAPI и async SQLAlchemy проверяет Telegram `initData`, синхронизирует данные для напоминаний и обслуживает события, историю и обратную связь.
- **Telegram Mini App** на React, TypeScript и Vite предоставляет мобильный интерфейс.

Лекарства и настройки хранятся в браузере по модели local-first. Backend получает снимки лекарств и только те настройки, которые нужны для фоновых напоминаний и восстановления данных на других устройствах. Реальная история приёмов остаётся серверной и всегда привязана к событию напоминания.
